...
exports.forgotPassword = async (req, res, next) => {
  try {
    ...
    const passResetLink = `https://${rootUrl}/pages/reset-password/${resetToken}`;
...
exports.resetPassword = async (req, res, next) => {
  try {
    ...
    const recievedTokenEncrypted = crypto
      .createHash("sha256")
    ...
    const user = await User.findOne({
      encryptedResetToken: recievedTokenEncrypted,
    });
    if (!user) {
      throw httpError(
        401,
        "Invalid Password reset request. Please generate a new password reset link"
      );
    }
